...
> 顶层还有一个 **`ai-sdlc-master` 多 Agent 总编排器**：一条指令把 PRD → Dev → QA 当一整条链跑。它**不只写「继续」——每次派发都内联「派发方式 / 停止规则 / 通过→下一轮 / 返回上一层」四要素**，在以下硬中断点必须 halt 回真人、不得自行续跑：PRD 每步确认 / DEV 启动前 / DEV G3 低置信 / QA gate_2·gate_3 / phase 边界授权（auto-confirm 仅主对话模式可用，无人值守模式一律关闭）。
...
下游步骤只看文件、不读聊天历史，从根上杜绝上下文漂移。每步有**确认闸门**，没确认不进下一步。编排侧（ai-sdlc-master）还在 PRD 每步 / DEV 启动前 / phase 边界等节点设**硬中断点**，与派发 prompt 内联的停止规则一致——详见 `skills/ai-sdlc-master/SKILL.md` 的「派发指令公约」。
...
| **全自动跑飞** | Agent 自己拍板推进，没人拦 | **确认闸门 + 硬中断点**：关键节点必须人确认；编排器每次派发内联停止规则，auto-confirm 仅主对话模式可用、无人值守模式一律真人确认，Agent 不私自续跑 |
...
└── harness/                # dev-harness 依赖的治理母本
    ...
    ├── scripts/            # 机械门禁脚本（self_check / scope_check / pre-commit / snapshot / rollback / dev_gate_check）
